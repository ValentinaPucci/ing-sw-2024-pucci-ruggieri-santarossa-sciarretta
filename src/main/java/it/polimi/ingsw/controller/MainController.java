...
import it.polimi.ingsw.networking.rmi.remoteInterfaces.MainControllerInterface;

import java.rmi.RemoteException;
import java.util.ArrayList;
import java.util.List;

import static it.polimi.ingsw.networking.PrintAsync.*;
...
public class MainController implements MainControllerInterface{
    ...
    public synchronized GameControllerInterface joinFirstAvailableGame(GameListener lis, String nick) throws RemoteException {
        List<GameController> ris = runningGames.stream().filter(x -> (x.getStatus().equals(GameStatus.WAIT) && x.getNumOfPlayers() < DefaultValues.MaxNumOfPlayer)).toList();
        Player p = new Player(nick);
        if (ris.size() > 0) {
            try {
    ...
    public synchronized GameControllerInterface joinGame(GameListener lis, String nick, int idGame) throws RemoteException {
        ...
        if (ris.size() == 1) {
            if(ris.get(0).getStatus().equals(GameStatus.WAIT) ||
                    ((ris.get(0).getStatus().equals(GameStatus.RUNNING) || ris.get(0).getStatus().equals(GameStatus.LAST_TURN)) &&
